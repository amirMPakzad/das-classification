...
# -----------------------------
# Chunk 1: FFT feature function (fallback)
# -----------------------------
def fallback_fft_features(w: np.ndarray) -> np.ndarray:
    """
    ...
    mag = np.log10(mag + 1.0)  # log scaling
    return mag


def compute_features(window_1d: np.ndarray, sample_len: int | None) -> np.ndarray:
    """
    Compute feature vector for one 1D window.
    """
    feat = fallback_fft_features(window_1d)
...
# -----------------------------
# Chunk 3: Load split sample indices
# -----------------------------
def load_split_samples(split_dir: Path, split_name: str) -> np.ndarray:
    """
    Loads splits/{train,val,test}_samples.npy, expected shape (N,3):
      [position, channel, label]
    We only need position/channel here.
    """
...
# -----------------------------
# Chunk 4: Iterate windows from split indices + raw H5
# -----------------------------
def iter_split_windows(
    ...
    """
    Yields (pos, ch, window_1d) for each (pos,ch) row in split_samples.
    Window extraction rule:
        start = pos * shift
        end   = start + fsize
        window = RawData[start:end, ch]
    """
...
# -----------------------------
# Chunk 5: Write one split to Torch .pt
# -----------------------------
def build_split_torch(
    raw_h5_path: Path,
    split_dir: Path,
    out_dir: Path,
    split_name: str,
    ...
    feat_dim = int(compute_features(dummy, sample_len).shape[0])

    # Output folder: out_dir/class_name/
    class_out_dir = out_dir / class_name
    class_out_dir.mkdir(parents=True, exist_ok=True)

    # e.g. train_regular.pt
    recording_name = raw_h5_path.stem.split("_")[0]
    recording_dir = class_out_dir / recording_name
    recording_dir.mkdir(parents=True, exist_ok=True)
    out_path = recording_dir / f"{split_name}_{class_name}.pt"

    # Pass A: gather valid (pos, ch)
    valid_meta: list[tuple[int, int]] = []
    ...
    # Pre-allocate torch tensors (CPU)
    x = torch.empty((n, feat_dim), dtype=torch.float32)
    y = torch.full((n,), int(class_id), dtype=torch.int64)
    pos_t = torch.empty((n,), dtype=torch.int32)
    ch_t = torch.empty((n,), dtype=torch.int32)
    ...
    with h5py.File(raw_h5_path, "r") as f:
        ...
        for pos, ch in valid_meta:
            ...
            # Safety: apply same filter again
            if drop_noise and not window_noise_ok(w, keep_if_regular=keep_if_regular):
                continue

            feat = compute_features(w, sample_len)
            if feat.shape[0] != feat_dim:
                raise RuntimeError(
                    f"Feature dim mismatch: got {feat.shape[0]} expected {feat_dim}"
                )

            x[out_i] = torch.from_numpy(feat)
            pos_t[out_i] = int(pos)
            ch_t[out_i] = int(ch)
            out_i += 1
    ...
    if out_i != n:
        raise RuntimeError(
            f"Wrote {out_i} samples but expected {n}. Filtering mismatch?"
        )

    payload = {
        "x": x,       # float32 [N, F]
        "y": y,       # int64   [N] (numeric label)
        "pos": pos_t, # int32   [N]
        "ch": ch_t,   # int32   [N]
        "meta": {
            "raw_h5": raw_h5_path.name,
            "split_name": split_name,
            "class_id": int(class_id),
            "class_name": class_name,
            "fsize": int(fsize),
            "shift": int(shift),
            "sample_len": int(sample_len) if sample_len is not None else None,
            "drop_noise": bool(drop_noise),
            "keep_if_regular": bool(keep_if_regular),
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Build train/val/test Torch datasets (.pt) from leakage-safe split npy + raw H5"
    )
    ...
    p.add_argument("--out-dir", type=Path, required=True, help="Output directory (will create out_dir/{class_name}/)")

    p.add_argument("--fsize", type=int, default=8192, help="Window size (raw samples). MUST match split config.")
    ...
    p.add_argument("--drop-noise",default=True, action="store_true", help="Apply spectral noise filtering")
    p.add_argument(
        "--keep-if-regular",
        action="store_true",
    ...
    p.add_argument("--class-id", type=int, required=True, help="Numeric label id (e.g. 0,1,2...)")
    p.add_argument("--class-name", type=str, required=True, help="Class name (e.g. regular, eventA, crack...)")
